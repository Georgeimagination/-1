...
结构化事实索引：本卡共有 10 条 `FACT-M2GA-GV3-*` 事实，完整清单见下节及 `数据/facts.csv`。
...
| `SRC-M2-GA-G06` | 当前 v3 产品页 | `redundant_covered`；架构事实由 G01/G02 覆盖，产品总量移入实现对象待办 |

实现对象待办 为 `DEF-M2GA-GV3-01` 至 `DEF-M2GA-GV3-04`，目标是未来的 TPU v3 `silicon_package`，不是本架构对象。卡内事实均满足七目标 XOR；没有云配置、Pod 或派生存算比。正式合并状态：`accepted / reviewed`。