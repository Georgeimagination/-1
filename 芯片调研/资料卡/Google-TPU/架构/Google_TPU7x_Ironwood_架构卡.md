...
结构化事实索引：本卡共有 10 条 `FACT-M2GA-G7X-*` 事实，完整清单见下节及 `数据/facts.csv`；其中不含 D2D 架构事实。

## 结构化事实明细

本卡对应 10 条 正式事实。下列标识逐条回指 `数据/facts.csv`：
...
`SEARCH-M2GA-G7X-MOE-HW` 的结果为 `no_reliable_result`，`REQ-M2GA-G7X-MOE-HW=not_found`。四项芯片 total 要求 `REQ-M2GA-GOOGLE_TPU7X_ARCH-*` 为 `not_applicable`。7300/7380 GB/s 视为显示精度或版本限定，不属于架构事实冲突；两者只在实现对象待办 留痕。
...
| `SRC-M2-GA-G04` | Hot Chips 37 演讲 | 架构事实层 `redundant_covered`；双 die/HBM/I/O 实现信息转 待办 |
| `SRC-M2-GA-G12` | performance guide | `redundant_covered`；每 TensorCore 容量属于实现口径 |

实现对象待办 为 `DEF-M2GA-G7X-01` 至 `DEF-M2GA-G7X-07`。卡内没有 Pod 规模、三维部署 topology fact 或由相对量推算的绝对值；七目标 XOR 已检查。正式合并状态：`accepted / reviewed`。