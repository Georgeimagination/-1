...
结构化事实索引：本卡共有 12 条 `FACT-M2GA-GV4-*` 事实，完整清单见下节及 `数据/facts.csv`。

## 结构化事实明细

本卡对应 12 条 正式事实。下列标识逐条回指 `数据/facts.csv`：
...
实现对象待办 为 `DEF-M2GA-GV4-01` 至 `DEF-M2GA-GV4-05`。光交换与 Pod 内容只在边界日志保留，不进入 待办 的芯片事实。卡内事实满足七目标 XOR，没有系统聚合和架构差异结论。正式合并状态：`accepted / reviewed`。