...
| 完整名称与配置边界 | Huawei Ascend 950PR | 含多个资源档位；未公开完整销售 part number 与各字段的一一配置映射 | `[1, PDF pp.13-15，正文 pp.9-11，表3-1及前文]` |
...
| Package | 2 个 AI Die + 2 个 IO Die + 8 个高速内存模块 | HiBL 1.0 内存分支 | 本卡正式物理对象；内存模块数不等于 DRAM 裸片层数 | `[1, PDF p.12，正文 p.8]` `[2, Ascend 950PR]` |
| 产品配置 | Ascend 950PR 的多个资源档位 | 使能资源与内存配置存在差异 | 按表3-1分别记录，不强行配对各字段 | `[1, PDF pp.13-15，正文 pp.9-11，表3-1]` |
| 上层产品 | Atlas 350 PCIe accelerator card | 搭载 950PR，卡级配置按 Atlas 350 资料确认 | 用于说明产品关系、实际系统配置和状态；不混入单封装聚合规格 | `[4, 新品发布]` |

完整 Ascend 950 设计包含 36 个 AI 子系统、4 个 AI CPU Cluster、4 个 DVPP 图像处理子系统、128MB L2 和 STARS2.0 调度系统。950PR 的实际公开计算档位为 32 / 28 个 Cube、64 / 56 个 Vector；“完整设计”与“产品使能资源”应分开记录。每个完整 CPU Cluster 包含 2 个 Linx816 CPU 和 4MB L3。[1, PDF p.13，正文 p.9，表3-1及前文]
...
|---|---|---|---|
| 完整设计与使能 | 完整设计 36 个 AI 子系统；本产品表列 32 / 28 Cube、64 / 56 Vector | 产品档位见§5，不能用完整设计替代使能数 | `[1, PDF p.13，正文 p.9，表3-1及前文]` |
...
下表按官方列顺序记录。950PR 的算力列依次对应 32 / 28 个 Cube，Vector 数为其两倍。这里只能把计算资源与同列计算规格对应；不能进一步按位置强行配对 CPU、DRAM 或 L2 档位。表3-1未给运行频率、功耗模式、FMA计数和结构化稀疏条件；以下是厂商规格值，不是实测。[1, PDF pp.13-15，正文 pp.9-11，表3-1]

| 单元/精度 | 原始值 | 口径 | 来源 |
|---|---:|---|---|
| Cube MXFP4 | 1730 / 1513 TFLOPS | 纯 Cube 规格，不含 Vector | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Cube HiF8 / MXFP8 / FP8 | 865 / 756 TFLOPS | 纯 Cube 规格，不含 Vector | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Cube INT8 | 865 / 756 TOPS | 纯 Cube 规格，不含 Vector | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Cube BF16 / FP16 | 432 / 378 TFLOPS | 纯 Cube 规格，不含 Vector | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Cube TF32 | 216 / 189 TFLOPS | 纯 Cube 规格，不含 Vector | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Vector FP16 / BF16 | 54 / 47 TFLOPS | 纯 Vector 规格，不含 Cube | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Vector FP32 | 27 / 23 TFLOPS | 纯 Vector 规格，不含 Cube | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Vector INT8 | 54 / 47 TOPS | 纯 Vector 规格，不含 Cube | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Vector INT16 | 27 / 23 TOPS | 纯 Vector 规格，不含 Cube | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Vector INT32 | 13 / 11 TOPS | 纯 Vector 规格，不含 Cube | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
| Vector INT64 | 6 / 5 TOPS | 纯 Vector 规格，不含 Cube | `[1, PDF pp.13-14，正文 pp.9-10，表3-1]` |
...
|---|---:|---|
| MXFP4 | 1784 / 1561 TFLOPS | `[1, PDF p.13，正文 p.9，表3-1]` |
| HiF8 / MXFP8 / FP8 | 919 / 804 TFLOPS | `[1, PDF p.13，正文 p.9，表3-1]` |
| INT8 | 919 / 804 TOPS | `[1, PDF p.13，正文 p.9，表3-1]` |
| BF16 / FP16 | 486 / 425 TFLOPS | `[1, PDF p.13，正文 p.9，表3-1]` |
| TF32 | 243 / 212 TFLOPS | `[1, PDF p.13，正文 p.9，表3-1]` |

产品页的最大1784TFLOPS对应32 Cube配置、MXFP4项的Cube+Vector聚合总量；同档纯Cube MXFP4为1730TFLOPS。引用1784TFLOPS时需保留Cube+Vector聚合口径，不能称为纯矩阵峰值。[1, PDF pp.13-14，正文 pp.9-10，表3-1] [3, 超强算力]

### 5.2 内存、接口与其他配置

| 字段 | 原始值或公开能力 | 条件和口径 | 来源 |
|---|---|---|---|
| 高速内存 | HiBL 1.0 HBM；白皮书规格为 128 / 112 GB | 多个processor配置；未公开与各Core/CPU档位的完整组合映射 | `[1, PDF pp.14, 25，正文 pp.10, 21，表3-1、表4-2]` `[2, Ascend 950PR]` |
| 内存带宽 | 1.6 / 1.4 TB/s | 单封装规格；读写合计方式、方向与持续带宽未说明 | `[1, PDF p.14，正文 p.10，表3-1]` |
...
| 存储层次 | SRAM/L2容量与512B line、128B sector已确认 | Register File总容量、cache各操作完整一致性规则及实测带宽未公开 | `[1, PDF pp.17, 25-27，正文 pp.13, 21-23]` `[5, 三级内存层次]` |
...
| 上市与供货 | Atlas350正式上市可确认 | 独立封装销售和全部资源配置的供货日期未知；白皮书无明确发布日期，不用版权年证明GA | `[4, 新品发布：Atlas 350 加速卡上市]` `[1, 版权页]` |
