...
官方虚拟化资源表给 64GB 物理卡列出 20/20 个可用 AI Core，但 exact-SKU 产品页和彩页没有公开 Matrix/Vector/Scalar 组织、L0/L1/L2/Unified Buffer 容量、程序员可见累加格式、物理 accumulator 位宽、结构化稀疏机制或内部 data mover。560TOPS INT8、280TFLOPS FP16 和 75TFLOPS FP32 不能反推出单 core 吞吐或时钟。[1, Technical Specifications] [2, Product Specifications] [4, Appendix Table 1] 用户指南系统框图明确说明处理器提供 Cache、图形/视频加速器与 I/O 资源，但没有给出 Cache 的层级、容量、带宽或延迟。[5, 系统框图]
...
| PCI identity | Vendor ID 0x19E5、Device ID 0xD802、Subsystem Vendor ID 0x19E5、Subsystem Device ID 0x4001 | 0x4000 对应 32GB，0x4001 对应 64GB；不能据此反推芯片子型号 | `[5, 表3-1]` |
...
vNPU 是从物理 NPU 硬切分出的虚拟实例。下表为本容量 SKU 的官方模板，资源数属于软件可分配额度；VPC 是图像预处理、VDEC 是视频解码，JPEGD/JPEGE 是 JPEG 解码/编码。所有模板的 PNGD 和 VENC 均为 0。[4, 附录“查询切分模板”表1]
...
`vir05_1c_16g` 的 VDEC 为 0；四分之一 AI Core 和内存份额并不意味着所有媒体资源也都有四分之一。64GB 附录没有列出 32GB 表中的 `_m`、`_nm` 模板，只能记录披露范围，不能据此断言不支持。[4, 附录“查询切分模板”表1]
...
| 虚拟化、RAS 与安全 | 硬切分模板、HBM ECC 与安全启动已确认 | 64GB SKU 模板见第 5.1 节；安全启动来自用户指南“概述” | 不将硬切分等同于 SR-IOV；ECC 粒度、page retirement、信任根和安全验证链仍未公开 |
