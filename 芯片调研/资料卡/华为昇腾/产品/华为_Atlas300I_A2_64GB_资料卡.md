...
本卡以一块配备 64GB HBM 的 Huawei Atlas 300I A2 inference card 为正式比较对象。它是双槽、全高全长的 PCIe 加速卡，面向服务器内 AI inference。32GB HBM 配置是同一产品名下的另一官方 SKU；Atlas 300I、300I Pro、300I Duo、Atlas 800I A2 server 和 A2 training products 均不与本卡合并。
...
| 完整 SKU | Huawei Atlas 300I A2 64GB | 当前产品页同时列 32GB 与 64GB，容量是区分本卡的正式配置字段；未找到独立 model/part number | `[1, Technical Specifications]` `[2, Product Specifications]` |
...
| 发布与当前状态 | 当前华为企业产品目录列出 64GB 配置并提供项目咨询，昇腾支持文档仍单列该产品；64GB 配置的独立发布日未找到 | 可确认当前目录和支持状态，不能由网页存在反推首批 shipment 日期或区域供货 | `[1, full page]` `[3, applicable hardware]` |

本卡包含 64GB HBM、1.6TB/s、AI/CPU 峰值、媒体能力、PCIe 5.0、350W 最大功耗、被动风冷、温度与卡尺寸。32GB 配置的 0.8TB/s 和专属硬切分模板不进入本卡，server drawer 的风扇数量、主机 CPU、系统内存、网络、整机功耗和多卡聚合性能也不属于本卡。

## 2. 层级关系与复用

| 层级 | 本 SKU 对应对象 | 是否与其他 SKU 共享 | 本卡怎么使用 | 来源 |
|---|---|---|---|---|
| Core IP | 64GB 物理卡报告 20/20 个可用 AI Core；内部执行组织和实际时钟未公开 | 32GB 与 64GB 配置的可用 AI Core 数相同 | 不用原始 Ascend 910、910B 的第三方参数反推单 core 结构 | `[4, Appendix: Querying specifications of a single NPU card, Table 1]` |
...
官方虚拟化资源表给 64GB 物理卡列出 20/20 个可用 AI Core，但 exact-SKU 产品页和彩页没有公开 Matrix/Vector/Scalar 组织、L0/L1/L2/Unified Buffer 容量、程序员可见累加格式、物理 accumulator 位宽、结构化稀疏机制或内部 data mover。560TOPS INT8、280TFLOPS FP16 和 75TFLOPS FP32 不能反推出单 core 吞吐或时钟。[1, Technical Specifications] [2, Product Specifications] [4, Appendix Table 1]
...
| HBM physical organization | 未公开 generation、stack 数、bus width、memory clock 或 package topology | 只记录 card/SKU 的 capacity、bandwidth 与 ECC | `[1, Technical Specifications]` `[2, Product Specifications]` |
...
| 可用加速资源 | 20 AI Core、7 AI CPU、9 VPC、2 VDEC、24 JPEGD、4 JPEGE；VENC 为 0，PNGD 不适用 | 官方 physical-card resource query；7 AI CPU 是可切分资源数，不替代产品页的 8-core physical CPU field | `[4, Appendix: Querying specifications of a single NPU card, Table 1]` |
| HBM | 64GB、1600GB/s（产品页写 1.6TB/s）、ECC | 本卡正式 SKU；不采用 32GB SKU 的 0.8TB/s | `[1, Technical Specifications]` `[2, Product Specifications]` |
| Host interface | PCIe 5.0；x16 为原指南摘录，待重核 | payload bandwidth、CXL、P2P 和 SR-IOV 未公开 | `[1, Technical Specifications]` `[5, Table 3-1]` |
| PCI identity | 待重核：Vendor ID 0x19E5、Device ID 0xD802、Subsystem Vendor ID 0x19E5、Subsystem Device ID 0x4001 | 0x4001 是 64GB 配置；32GB 配置为 0x4000 | `[5, Table 3-1]` |
| 功耗 | 最大 350W 已确认；300W 配置为原指南摘录，待重核 | 用户指南未把 300/350W 映射到特定容量，因此不声称 64GB 必然为 350W | `[1, Technical Specifications]` `[2, Product Specifications]` `[5, Table 3-1]` |
...
| 硬切分 | 官方附录为 64GB SKU 提供 1-to-2 与 1-to-4 inference partition 模板 | `vir10_3c_32g` 分配 10 AI Core/32GB/3 AI CPU，`vir05_1c_16g` 分配 5 AI Core/16GB/1 AI CPU；适用 HDK 25.5.0 与 CANN 9.0.0-beta.1 | `[4, Sections 1.3, 2.1 and Appendix Table 1]` |
...
|---|---|---|---|
| 64GB 与 32GB | 同一产品名下的两个 official configuration | 64GB 为 1.6TB/s；32GB 为 0.8TB/s，其他 card-level headline 在同一表中共用 | 本卡只采用 64GB/1.6TB/s 和对应切分模板，32GB 另有 SKU 卡 |
| FP32 峰值 | 官方页面披露粒度不同 | 彩页列 75TFLOPS；当前网页规格表只列 INT8/FP16 | 保留彩页的 75TFLOPS，并注明来源差异 |
| PCIe | 代际和 lane width 均可由官方资料确认 | 产品页和彩页写 PCIe 5.0，用户指南给 x16 | 采用 exact-product official sources，不使用生态伙伴参数 |
...
| 虚拟化、RAS 与安全 | 硬切分模板已公开，其他项未公开 | 64GB SKU 支持 1-to-2/1-to-4 inference hard partition；HBM ECC 可确认 | 不把硬切分解释成 SR-IOV；secure boot、page retirement、ECC 粒度等保持未公开 |
| 发布日期、独立料号与首批出货 | 未找到 | 当前产品页、彩页和支持文档 | 只记录当前官方列出的 64GB product configuration |
...
| `[4]` | Huawei Ascend Community，*NPU 卡虚拟化硬切分参考实践* | 官方技术文章 | 64GB 卡的可用 AI/媒体资源、1-to-2/1-to-4 partition templates 及版本条件 | <https://www.hiascend.com/zh/developer/techArticles/20251212-1> |
...
复核结论：Huawei Atlas 300I A2 64GB 的正式主语是一块双槽、全高全长、被动风冷的 PCIe inference card。64GB HBM、1600GB/s、ECC、560TOPS INT8、280TFLOPS FP16、75TFLOPS FP32、1 个官方统称的 Ascend 910 processor、20 个 AI Core、8 个 2.0GHz TaiShan Core、PCIe 5.0 x16、300W/350W 功耗配置、最大 350W、5°C 至 45°C、卡尺寸和 64GB 专属硬切分模板分别引自华为一手资料；其中仅由用户指南 [5] 支持的字段，本轮未能重新打开原文，仍待复核。32GB/0.8TB/s 配置没有下放；drawer 风扇也没有写成 card component。exact chip submodel、工艺、die/package、单 core 结构、内部 cache/scratchpad、数值与稀疏语义、其他 RAS、安全和专用 LLM 单元保持未公开。
