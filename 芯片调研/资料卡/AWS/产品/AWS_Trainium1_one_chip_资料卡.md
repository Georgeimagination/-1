...
| TensorEngine | 128×128 PE systolic array；每个NCv2为92TFLOPS FP16/BF16和23TFLOPS FP32，完整芯片当前官方规格为190与47.5TFLOPS | TensorE数据路径2×128 input、1×128 output，频率2.8GHz；Core值与chip headline存在小幅口径差 | `[2, Tensor Engine and engine width/frequency table]` |
...
| 稀疏与专用单元 | 未找到结构化稀疏Tensor path或专用MoE router；chip级有6个CC-Core用于collective communication | MoE是系统目标workload，不代表芯片含独立router | `[2, device overview]` `[5, High-performance networking]` |
...
| 设备互联端点 | 4个NeuronLink-v2接口；16-chip实例产品页写最高768GB/s NeuronLink | 一芯片`trn1.2xlarge`为N/A；方向、per-interface速率和有效负载未公开 | `[2, device overview]` `[5, NeuronLink interconnect and Product details]` |
...
| NCv2与chip峰值 | 官方直接值不能简单相加 | NKI指南给每个TensorE 92TFLOPS FP16/BF16、23TFLOPS FP32；当前chip页给两核合计190与47.5 | 芯片字段用direct per-chip值，Core值只在Core层记录 |
