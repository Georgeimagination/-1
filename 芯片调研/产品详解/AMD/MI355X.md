...
MI355X 是 CDNA 4 家族中面向高密度系统的 OAM（OCP Accelerator Module，加速器模组）。它与 MI350X 共享主要计算封装和 HBM 组织，通过更高频率与更大的供电、冷却预算提高理论吞吐，覆盖 AI training、inference 与高性能计算。[2, p.1] [3, pp.3-4]

![MI355X 架构示意](MI355X-架构.png)
...
MI355X 有 8 个 TSMC N3P XCD。每个 XCD 设计 36 个 CU（Compute Unit，计算单元），实际启用 32 个，整颗 GPU 合计 256 个 CU、16,384 个 Stream Processor 和 1,024 个 Matrix Core，最高引擎频率 2.4GHz。产品页列出的总晶体管数为 1,850 亿。CU 数量不直接代表跨代算力：CDNA 4 的每个 CU 增强了矩阵运算和低精度格式支持，不能只按 CU 数比较上一代与这一代。[3, pp.4-8] [1, GPU Specifications]
...
HBM 中的模型数据经内存系统供给计算，重复使用的数据可由 L2、Infinity Cache 或程序安排的 LDS 复用。计算分区可以沿 XCD 划为 1、2、4、8 份；内存访问则有 NPS1 与 NPS2 两种 NUMA（非均匀内存访问）模式，分别跨两 IOD 交织或形成两个 144GB 内存池。产品 brochure 的 Memory Partitions 另写“1 or 4”，与白皮书的 NPS1/NPS2 不一致，因此不能直接把该字段解释为 NPS4。[3, pp.11-13, Figure 6] [2, p.1]
...
|---|---:|---:|
| MXFP4 / MXFP6 | 各 10.0663PFLOPS | 未列 |
| MXFP8 | 5.0332PFLOPS | 未列 |
| OCP-FP8 Matrix | 5.0332PFLOPS | 10.0664PFLOPS |
| INT8 Matrix | 5.0332POPS | 10.0664POPS |
| FP16 / BF16 Matrix | 各 2.5166PFLOPS | 各 5.0332PFLOPS |
| FP16 Vector | 157.3TFLOPS | 不适用 |
| FP32 Vector | 157.3TFLOPS | 不适用 |
| FP32 Matrix | 157.3TFLOPS | 未列 |
| FP64 Vector | 78.6TFLOPS | 不适用 |
| FP64 Matrix | 78.6TFLOPS | 未列 |

表内采用 datasheet 的精确显示值，产品页的 10.1/5.0PFLOPS 等数字为舍入显示。表中 PFLOPS 为每秒千万亿次浮点运算，POPS 用于整数操作，Vector 与 Matrix 路径不相加。[1, GPU Specifications] [2, p.1 AI / HPC Peak Theoretical Performance]
...
MI355X 的 TBP（总板级功耗）为 1400W，产品页列 Passive & Active 冷却，白皮书 Table 2 列 Passive/Liquid，并重点介绍直接液冷的高密度系统。DLC（Direct Liquid Cooling，直接液冷）是该产品的重要部署方案，但这些资料并未将每个 MI355X OAM 限定为唯一一种冷却实现；所查来源未给出冷板、入口温度、流量与压降。[1, Requirements / Board Specifications] [2, p.1] [3, p.3, Table 2]
...
[1] AMD，*AMD Instinct MI355X GPUs*。<https://www.amd.com/en/products/accelerators/instinct/mi350/mi355x.html> [本地原文](../../原始资料/网页快照/AMD/产品详解补充/2026-09-17/MI355X-1-mi355x.html)

[2] AMD，*AMD Instinct MI355X GPU*，LE-92703-00，10/25。<https://www.amd.com/content/dam/amd/en/documents/instinct-tech-docs/product-briefs/amd-instinct-mi355x-gpu-brochure.pdf> [本地原文](../../原始资料/论文/AMD_Instinct/90_官方白皮书与技术资料/amd-instinct-mi355x-gpu-brochure.pdf)
