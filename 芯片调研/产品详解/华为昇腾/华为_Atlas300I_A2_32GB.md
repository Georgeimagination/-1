# 华为 Atlas 300I A2 32GB

Atlas 300I A2 是安装在服务器中的 PCIe 推理加速卡，面向生成式模型、推荐、搜索和内容审核。32GB 型号将 AI 计算、通用 CPU、媒体处理与 HBM 高带宽内存放在同一张卡上；产品指南将用途明确限定为推理。理解这张卡时，既要看模型计算能力，也要看输入数据如何进入处理器、媒体预处理占用什么资源，以及服务器能否提供所需电力与气流。[1, 概述] [2, 应用场景] [5, 产品简介“概述”]

![Atlas 300I A2 32GB 板卡功能架构](华为_Atlas300I_A2_32GB-架构.png)
...
处理器集成 20 个 AI Core 和 8 个 TaiShan Core，彩页给出 CPU 频率 2.0GHz。AI Core 执行神经网络计算，CPU 处理通用任务，图像与视频单元承担模型输入前后的媒体工作。官方虚拟化资源查询另列 7 个可分配 AI CPU；这是软件资源池的数量，与八核物理 CPU 规格属于不同层次。[5, 图1-2、表3-1] [2, 产品规格] [4, 附录“查询单NPU卡规格”表2]
...
卡通过 PCIe 5.0 ×16 接口与主机通信。板上的 MCU 是管理微控制器，服务器 iBMC 可以通过它读取 PCB、BOM 版本，以及单板温度、功耗、电源电压；指南还列出带内在线升级和带内、带外状态查询。这里的管理通路与承载模型数据的 PCIe 通路作用不同。[5, 系统框图、表3-1、可维护性] 维护文档为本卡提供 PCIe 信号质量查询，本组产品资料没有给出可用于推导 HCCS 或 RoCE 带宽的本卡专属接口规格。[3, 眼图测试参数] 用户指南概述还确认安全启动，但没有展开本卡信任根、密钥和验证链的实现。[5, 产品简介“概述”]
...
| HBM | 32GB，800GB/s，支持 ECC | 标称容量和带宽，ECC 为存储纠错。[5, 表3-1] |
...
官方资源查询表列出整卡可分配的 20 AI Core、32GB 内存、7 AI CPU、9 VPC、2 VDEC、24 JPEGD、4 JPEGE。VPC 是图像预处理资源，VDEC 是视频解码，JPEGD/JPEGE 分别是 JPEG 解码/编码；VENC（视频编码）为 0，PNGD（PNG 解码）为不适用。这里描述的是软件可分配资源，并没有给出每个资源背后的物理流水线组织。[4, 附录“查询单NPU卡规格”表2]
...
表中 vNPU 指从物理卡硬切分出的虚拟 NPU；所有模板的 PNGD 和 VENC 均为 0。同样是 10 AI Core 和 16GB 内存，带 `_m` 的模板取得整卡全部可分配媒体资源，`_nm` 模板则不取得媒体资源。因而不能简单把“半张卡”理解成每一种资源都除以二；若采用含 5 AI Core、8GB 的模板，可以进一步形成四个实例。[4, 附录“查询切分模板”表2]
...
主机识别本卡时，PCI Vendor ID 与 Subsystem Vendor ID 均为 `0x19E5`，Device ID 为 `0xD802`，本 32GB 配置的 Subsystem Device ID 为 `0x4000`。这些标识有助于区分同名的两款板卡，但不能单独推断处理器微架构。准确芯片子型号、独立首发日期、首次出货时间，以及尚未展开的片内层级带宽与延迟，仍未在本文所用资料中找到。[5, 表3-1] [1, 产品页] [2, 产品规格]
