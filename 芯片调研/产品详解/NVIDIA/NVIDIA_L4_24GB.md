...
L4 是面向 AI 推理、视频和图形工作负载的低功耗 Ada 加速卡。它采用板上 GDDR6，而非与计算裸片同封装的 HBM（高带宽堆叠内存），经 GPU 的内存接口访问。[1, pp.1-3；2, pp.39-40；4, L4] 产品简报也列出 training，因此“偏推理”描述主要设计与推广取向，并不表示训练不受支持。[1, p.1, Overview]
