...
结构化事实索引：本卡全部 17 条 `FACT-M2GA-ATRN1-*` 机制/数值语义事实；完整清单见 `structured/facts.csv`。
...
| `SRC-M2-GA-A03` | Trainium 产品身份、cFP8/TF32 与内联压缩/解压缩 | 保留 identity、数值格式和数据搬运机制；芯片总量转入 backlog |
| 更早 Neuron 页 | 旧 420 INT8 TOPS | `superseded` 线索；不进入事实链，也不猜差值原因 |

实现 backlog 为 `DEF-M2GA-ATRN1-01` 至 `DEF-M2GA-ATRN1-05`，目标是 Trainium1 silicon_package。EC2 Trn1 实例与系统聚合不下放。七目标 XOR 已检查；没有派生存算比。当前自检状态：`draft / ready_for_parent_review`。