...
| `SRC-M2-GA-G14` | 状态和 ICI 披露 | 保留状态与接口存在性；19.2 Tb/s 转入实现 backlog |

实现 backlog 为 `DEF-M2GA-G8I-01` 至 `DEF-M2GA-G8I-05`。Boardfly 的四芯片环、全连接组、光电路交换与 Pod 尺度只在 `M2GA-BD-001` 留边界记录。卡内没有芯片/系统聚合，也没有由峰值反推阵列；七目标 XOR 已检查。当前自检状态：`draft / future_explicit_only`。