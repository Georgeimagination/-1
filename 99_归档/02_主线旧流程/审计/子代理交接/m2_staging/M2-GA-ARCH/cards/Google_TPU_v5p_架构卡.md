...
结构化事实索引：本卡共有 11 条 `FACT-M2GA-GV5P-*` 事实，完整清单见下节及 `structured/facts.csv`。

## 结构化事实明细

本卡对应 11 条 staging 事实。下列标识逐条回指 `structured/facts.csv`：
...
实现 backlog 为 `DEF-M2GA-GV5P-01` 至 `DEF-M2GA-GV5P-06`。卡内事实通过七目标 XOR 检查，未生成存算比，因为算力、容量和带宽不属于本对象。当前自检状态：`draft / ready_for_parent_review`。