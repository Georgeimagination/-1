# Google TPU v6e（Trillium）架构卡（草稿）

- 对象：`OBJ-GOOGLE-TPU-V6E-ARCH`
- 层级：`architecture_generation`
- 资料截止日：2026-08-12
- 边界：动态产品页中的核心数、峰值、容量和带宽只进实现 backlog；本卡保留 v6e 的程序员可见机制。
...
结构化事实索引：本卡共有 8 条 `FACT-M2GA-GV6E-*` 事实，完整清单见下节及 `structured/facts.csv`。
...
| `SRC-M2-GA-G10` | v6e 身份与 ICI architecture | 保留；芯片总量转入 backlog，网页仍需快照 |
| `SRC-M2-GA-G15` | SparseCore 的本地存储、动态执行、向量处理及跨 lane 操作 | 保留特殊能力独有证据；网页需快照，不据此断言存储管理方式 |

实现 backlog 为 `DEF-M2GA-GV6E-01` 至 `DEF-M2GA-GV6E-04`，目标是 v6e cloud_accelerator。卡内事实满足七目标 XOR，未使用系统聚合或媒体材料。当前自检状态：`draft / explicit_gaps`。