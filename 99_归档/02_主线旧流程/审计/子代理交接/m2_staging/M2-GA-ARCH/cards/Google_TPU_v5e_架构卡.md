# Google TPU v5e 架构卡（草稿）

- 对象：`OBJ-GOOGLE-TPU-V5E-ARCH`
- 层级：`architecture_generation`
- 资料截止日：2026-08-12
- 边界：v5e 的公开材料以 Google Cloud 动态文档为主。本卡只固定架构机制，产品页中的芯片总量放入实现 backlog。

## 架构事实

v5e 的 TensorCore 包含 MXU、VPU 和标量/控制路径；MXU 的公开架构口径是 128×128 脉动阵列（`FACT-M2GA-GV5E-EXEC`、`FACT-M2GA-GV5E-ARRAY`）。BF16 矩阵输入采用 FP32 累加语义（`FACT-M2GA-GV5E-BF16-IN`、`FACT-M2GA-GV5E-BF16-ACC`）。产品页还列出 INT8 支持，但本包没有把芯片总 TOPS 写入架构对象。
...
结构化事实索引：本卡共有 6 条 `FACT-M2GA-GV5E-*` 事实，完整清单见下节及 `structured/facts.csv`。

## 结构化事实明细

本卡对应 6 条 staging 事实。下列标识逐条回指 `structured/facts.csv`：
...
| `SRC-M2-GA-G08` | v5e 页标题和 interconnect architecture | 保留，限定对象身份并确认 ICI 入口；芯片总量转入 backlog |

目前没有能替代 G08 的固定 v5e 架构论文。实现 backlog 为 `DEF-M2GA-GV5E-01` 至 `DEF-M2GA-GV5E-04`，建议目标是 TPU v5e `cloud_accelerator`。卡内事实满足七目标 XOR，未使用 Pod 聚合值或媒体来源。当前自检状态：`draft / explicit_gaps`。