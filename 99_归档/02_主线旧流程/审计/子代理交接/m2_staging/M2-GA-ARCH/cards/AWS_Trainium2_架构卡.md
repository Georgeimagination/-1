...
在不复制正式事实的前提下，本卡只把“NCv3 采用异构 Tensor/Vector/Scalar/GpSimd 路径、软件管理片上存储、显式 DMA、CC-Core/NeuronLink collective offload，以及 AWS Neuron/NKI 编程模型”作为当前实现所显示的机制候选。它们仍须由总控决定是否以新的、直接归属架构对象的断言入库；本 staging 没有自动上卷。芯片峰值、执行单元数量、SBUF/PSUM/HBM 容量和带宽、链路数、频率、封装及所有派生存算比绝不属于这组候选。
## 结构化事实明细

本卡不新增 staging 事实；Trainium2 仅引用正式库既有事实，映射见 sources/trainium2_existing_reuse_map.csv。
...
实现 `backlog` `DEF-M2GA-ATRN2-01` 只说明现有事实应继续留在 `OBJ-AWS-TRAINIUM2-CHIP`。CC-Core 16/20 沿用 M1 `CF-006`，本包不复制也不宣告解决。实例、UltraServer 和系统聚合全部排除。七目标 XOR 的检查对象是引用映射，不新增目标列。当前自检状态：`draft / reference_existing_only`。