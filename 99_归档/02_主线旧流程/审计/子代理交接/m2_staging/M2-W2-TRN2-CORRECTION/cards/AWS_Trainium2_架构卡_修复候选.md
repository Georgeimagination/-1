...
上述内容不是 `OBJ-AWS-TRAINIUM2-ARCH` 的自身事实，也不证明以后所有 Trainium2 实现共享相同容量、峰值、核心/链路数或物理参数。修复后的引用映射共有 107 行，覆盖 106 个唯一 `fact_id`；同一 fact 的两行表示不同来源断言。候选文件为 `sources/trainium2_existing_reuse_map_corrected.csv`，每行仍标为 `reference_existing_fact_do_not_copy`。原映射中的三条 S10 BF16/FP16/TF32 芯片历史事实已删除。
...
| `SRC-AWS-TRN2-S10`、`S11` | 历史版本与发布日期 | S10 只保留 FP8 芯片历史值与发布状态；不扩写 BF16/FP16/TF32 |
