# NVIDIA Blackwell Ultra 架构资料卡（草稿）

> 对象：OBJ-NVIDIA-BLACKWELL-ULTRA-ARCH；层级：architecture_generation；事实冻结日：2026-08-12；复核状态：draft。


术语：SFU（Special Function Unit，特殊函数单元）承担指数等特殊函数；`MUFU.EX2` 是这里直接记录的指数指令路径。
## 对象边界

本卡只记录直接以 Blackwell Ultra 为主语的增量事实，不从 Blackwell 隐式继承。当前正式对象关系表没有允许复用共有事实的关系；是否新增关系由总控决定。在此之前，矩阵、存储和互联共有项都记为未直接披露或待关系复核，不判断这些能力在架构上不存在。
