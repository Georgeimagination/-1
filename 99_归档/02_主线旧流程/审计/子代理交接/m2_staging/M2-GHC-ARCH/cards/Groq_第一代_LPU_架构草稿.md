...
MEM 提供扁平的全局片上 SRAM，由编译器显式安排数据搬运。结构化事实只把管理方式规范为 `compiler_managed`，完整厂商表述保留在断言中。220 MiB、55 TiB/s、230 MB 和 80 TB/s 分属首颗实现或后续产品修订，均在 `implementation-object-backlog.csv` 中等待物理对象承接，不能用于计算架构代际存算比。
...
上述表覆盖本对象在 staging 中的 13 条事实。名称、组件职责、软件映射和数值路径都保留卡片入口，没有仅存在于 CSV 的孤立事实。
...
自检状态：`ready_for_independent_rereview`。工作包校验器已在修复后的 35 条事实、35 条断言和 80 条字段要求上通过；字段目标语义检查无错误，临时合并正式库后，官方校验器通过 46,867 项检查。