...
上述表覆盖本对象在 staging 中的 10 条事实。被移除的拓扑枚举不是证据丢失：厂商原词仍在 topology notes、字段要求和卡片正文中；per-LPU 数值保存在实现待办。
...
最小架构来源保留架构博客和 C2C 专题博客。前者支持执行组织、SRAM 管理和编译器映射；后者独有互联时序与路由机制。LPX 产品页对本包选定事实集没有新增内容，继续由 `COV-NVG3-PRODUCT-BY-ARCHBLOG` 标为 `fully_covered`。

自检状态：`ready_for_independent_rereview`。工作包校验器和字段目标语义检查均已通过；临时合并正式库后，官方校验器通过 46,867 项检查。