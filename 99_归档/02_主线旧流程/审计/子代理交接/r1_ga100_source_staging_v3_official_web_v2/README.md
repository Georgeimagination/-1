# GA100 v3 官方网页与固定文档快照暂存 v2

本目录是 `r1_ga100_source_staging_v3_official_web/` 的完整 v2 修正版。独立复核已拒收 v1，唯一 blocker 是 `R1GA100-WEB-010` 在 manifest 与 source-family map 中使用了不同的来源家族字符串。v2 将两处和 `capture-input.tsv` 统一为 `NVIDIA AI Enterprise Infra 8.2 support matrix`，保留 `SFAM-R1GA100-NVIDIA-AI-ENTERPRISE-INFRA-8-2`、endpoint 归属和所有 payload bytes 不变。v1 目录未改动；v2 当前状态为待独立验收。

本目录继续为 `R1-CHIP-NVIDIA-GA100` 的来源工程提供可复查的 NVIDIA 官方网页和 PDF 快照。它只服务总控后续的来源建模、事实断言和反向移除复核；没有改动 32 张正式表、未来表、既有来源表、资料池清单、模板、校验器或进度文件。
...
`README.md` 与 `validation_report.md` 已分别通过 `report-humanizer` 单文件扫描，并从结尾向前复读版本边界、候选状态、payload 类型、外链资源范围、v1 reject 与 v2 待验收边界。结果记录在 `validation_report.md`。
