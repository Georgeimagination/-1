...
$ExpectedValidatorChecks = 115509
$ExpectedCandidatePosthashSha256 = 'b84ce89dbdf96accb7861e61cedbfb33618ed8793d33188030f7ef51f15efe73'
$ExpectedCandidateAggregate = 'bcb16e932019c1625e9b5a5309a37f569c0bb230b7c921a589dd3f7175632b8c'
$ExpectedToken = 'ASCEND950-FORMAL-MERGE-20260813-0F59AA2F-F1520907'
$IsolatedMarkerText = 'M2-W3-HUAWEI-ASCEND950-ISOLATED-TRANSACTION'
...
$Preflight = Join-Path $WorkRoot 'Test-Ascend950FormalTransactionPreflight.ps1'
$ReplayEngine = Join-Path $WorkRoot 'Replay-Ascend950AuthorizedRows.ps1'
...
Assert-Check ($guardList.Count -eq 5) 'Guard list is not five rows.'
Assert-Check ($newFileList.Count -eq 8) 'New-file target list is not eight rows.'
...
if ($isFormalRoot) {
    ...
    Assert-Check ($transactionReviewRow.transaction_review_id -ceq 'ASCEND950-FORMAL-TRANSACTION-REVIEW-20260813') 'Transaction-review ID differs.'
    Assert-Check ($transactionReviewRow.package_id -ceq 'M2-W3-HUAWEI-ASCEND950-FORMAL-TRANSACTION-V1') 'Transaction-review package ID differs.'
    Assert-Check ($transactionReviewRow.reviewer -ceq 'm2_ascend950_formal_transaction_independent_review') 'Transaction-review reviewer identity differs.'
    Assert-Check ($transactionReviewRow.reviewer -cne 'm2_ascend950_transaction_prep') 'Transaction-prep author cannot independently authorize formal execution.'
    Assert-Check ($transactionReviewRow.review_date -ceq '2026-08-13') 'Transaction-review date differs.'
    Assert-Check ($transactionReviewRow.authorization_scope -ceq 'formal_execution_of_exact_553_row_signoff_via_transaction_v1') 'Transaction-review authorization scope differs.'
    ...
    $rehearsalScript = Join-Path $WorkRoot 'Test-Ascend950FormalTransactionRehearsal.ps1'
    ...
    Assert-Check ($transactionReviewRow.ascend950_signoff_sha256 -ceq $ExpectedSignoffSha256) 'Transaction-review ASCEND950 signoff binding differs.'
    Assert-Check ($transactionReviewRow.staging_manifest_sha256 -ceq $ExpectedStagingManifestSha256) 'Transaction-review Ascend 950 staging-manifest binding differs.'
    ...
    $backupManifestPath = Join-Path $BackupRoot 'ASCEND950-FORMAL-BACKUP-MANIFEST.csv'
    ...
    Assert-Check ($backupManifest.Count -eq 24) "Formal backup manifest row count differs: $($backupManifest.Count)."
    Assert-Check (@($backupManifest.relative_path | Sort-Object -Unique).Count -eq 24) 'Formal backup manifest paths are not unique.'
    $expectedBackupPaths = @($backupList.target_path) + @($guardList.target_path | Sort-Object -Unique) + @('ASCEND950-NEW-FILE-TARGETS-ABSENT.csv','ASCEND950-CANDIDATE-20-TABLE-POSTHASHES.csv')
    ...
    $absenceBackup = Get-ChildPath -Root $BackupRoot -RelativePath 'ASCEND950-NEW-FILE-TARGETS-ABSENT.csv' -MustExist $true
    Assert-Check ((Get-Sha256 $absenceBackup) -ceq (Get-Sha256 $NewFileListPath)) 'Formal backup absent-target list differs.'
    $posthashBackup = Get-ChildPath -Root $BackupRoot -RelativePath 'ASCEND950-CANDIDATE-20-TABLE-POSTHASHES.csv' -MustExist $true
    ...
    $authorizationMarker = Join-Path $BackupRoot 'ASCEND950-FORMAL-TRANSACTION-AUTHORIZATION.txt'
    Assert-Check (Test-Path -LiteralPath $authorizationMarker -PathType Leaf) 'Formal BackupRoot lacks ASCEND950-FORMAL-TRANSACTION-AUTHORIZATION.txt.'
...
if (-not $isFormalRoot) {
    $markerPath = Join-Path $TargetRoot '.ascend950-formal-transaction-mirror'
    Assert-Check (Test-Path -LiteralPath $markerPath -PathType Leaf) 'Non-formal TargetRoot lacks .ascend950-formal-transaction-mirror.'
    $markerText = [IO.File]::ReadAllText($markerPath, [Text.Encoding]::UTF8).Trim()
    Assert-Check ($markerText -ceq 'ASCEND950-FORMAL-TRANSACTION-MIRROR-20260813') 'Non-formal TargetRoot marker content differs.'
}
...
$transactionRoot = Join-Path $transactionContainer ('.tmp-ascend950-formal-' + [guid]::NewGuid().ToString('N'))
...
if (-not [string]::IsNullOrWhiteSpace($env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS)) { $FaultAfterReplacements = [int]$env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS }
if (-not [string]::IsNullOrWhiteSpace($env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS)) { $FaultAfterPayloads = [int]$env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS }
...
$lockPath = Join-Path $lockRoot ('ASCEND950-FORMAL-TRANSACTION-LOCK-' + $lockKey + '.lock')
$lockStream = $null
$lockOwned = $false
try {
...
try {
    ...
    [IO.File]::WriteAllText((Join-Path $candidateRoot '.ascend950-merge-isolated-target'), $IsolatedMarkerText, (New-Object Text.UTF8Encoding($false)))
    ...
    Assert-Check ([int]$replayResult.authorized_writes -eq 545) 'Accepted replay engine did not execute 545 writes.'
    Assert-Check ([int]$replayResult.files_created -eq 8) 'Accepted replay engine did not copy eight transaction files.'
    ...
    Assert-Check ($candidateAggregate -ceq $ExpectedCandidateAggregate) 'Candidate 32-table aggregate differs.'
    ...
    foreach ($item in $backupList) {
        ...
        $tempReplacement = $target + '.ascend950-formal-transaction-new'
    ...
    foreach ($item in $backupList) {
        $target = Get-ChildPath -Root $TargetRoot -RelativePath $item.target_path -MustExist $true
        Assert-Check (-not (Test-Path -LiteralPath ($target + '.ascend950-formal-transaction-new'))) "Replacement temp remains after success: $($item.target_path)"
        Assert-Check (-not (Test-Path -LiteralPath ($target + '.ascend950-formal-transaction-restore'))) "Restore temp remains after success: $($item.target_path)"
    }
    ...
    Add-JournalEvent -Phase 'commit_complete' -Kind 'transaction' -RelativePath '' -ObservedSha256 $candidateHashManifestSha256 -Details '545_writes_5_guards_115509_checks'
    ...
    try {
        ...
        for ($index = $replacedPaths.Count - 1; $index -ge 0; $index--) {
            ...
            $restoreTemp = $target + '.ascend950-formal-transaction-restore'
        ...
        foreach ($item in $backupList) {
            $target = Get-ChildPath -Root $TargetRoot -RelativePath $item.target_path -MustExist $true
            $replacementTemp = $target + '.ascend950-formal-transaction-new'
            $restoreTemp = $target + '.ascend950-formal-transaction-restore'
