...
param(
    ...
    [string]$PackageRoot = 'D:\Workspace\2026projects\调研-训练推理芯片调研-资料汇总\审计\子代理交接\m2_ascend950_formal_transaction_prep'
)
Set-StrictMode -Version 2.0
$ErrorActionPreference='Stop'
$Utf8NoBom=New-Object Text.UTF8Encoding($false)
$ExpectedBaseline='f1520907d54e406e7d82f591e3799f07f569218aff0c321ac3b836b6c649da10'
$ExpectedPostmerge='bcb16e932019c1625e9b5a5309a37f569c0bb230b7c921a589dd3f7175632b8c'
...
[IO.File]::WriteAllText((Join-Path $mirror '.ascend950-formal-transaction-mirror'),'ASCEND950-FORMAL-TRANSACTION-MIRROR-20260813',$Utf8NoBom)
...
$beforeFormal=Get-Aggregate $SourceRoot $manifest;$executor=Join-Path $PackageRoot 'scripts\Invoke-Ascend950FormalTransaction.ps1'
$env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS='';$env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS=''
if($Scenario-ceq'rollback_after_3_tables'){$env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS='3'}
if($Scenario-ceq'rollback_after_2_payloads'){$env:ASCEND950_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS='2'}
...
$csvCommits=@($journal|Where-Object{$_.phase-ceq'commit_complete'-and$_.kind-ceq'csv_replace'}|ForEach-Object{$_.relative_path})
$csvRollbacks=@($journal|Where-Object{$_.phase-ceq'rollback_complete'-and$_.kind-ceq'csv_restore'}|ForEach-Object{$_.relative_path})
$payloadCommits=@($journal|Where-Object{$_.phase-ceq'commit_complete'-and$_.kind-ceq'payload_copy'}|ForEach-Object{$_.relative_path})
$payloadRollbacks=@($journal|Where-Object{$_.phase-ceq'rollback_complete'-and$_.kind-ceq'payload_delete'}|ForEach-Object{$_.relative_path})
$expectedCsvReverse=@($csvCommits);[array]::Reverse($expectedCsvReverse)
$expectedPayloadReverse=@($payloadCommits);[array]::Reverse($expectedPayloadReverse)
$reverseCsvVerified=if($Scenario-ceq'positive'){$csvRollbacks.Count-eq0}else{($csvRollbacks-join[char]31)-ceq($expectedCsvReverse-join[char]31)}
$reversePayloadVerified=if($Scenario-ceq'positive'){$payloadRollbacks.Count-eq0}else{($payloadRollbacks-join[char]31)-ceq($expectedPayloadReverse-join[char]31)}
if(-not$reverseCsvVerified){Fail 'CSV rollback order is not the reverse of commit order.'}
if(-not$reversePayloadVerified){Fail 'Payload rollback order is not the reverse of commit order.'}
if($Scenario-ceq'positive'){
    ...
    if($csvCommits.Count-ne20-or$payloadCommits.Count-ne8-or$csvRollbacks.Count-ne0-or$payloadRollbacks.Count-ne0){Fail 'Positive journal commit shape differs.'}
    ...
    if($Scenario-ceq'rollback_after_3_tables' -and ($csvCommits.Count-ne3-or$csvRollbacks.Count-ne3-or$payloadCommits.Count-ne0-or$payloadRollbacks.Count-ne0)){Fail 'Three-table rollback journal shape differs.'}
    if($Scenario-ceq'rollback_after_2_payloads' -and ($csvCommits.Count-ne20-or$csvRollbacks.Count-ne20-or$payloadCommits.Count-ne2-or$payloadRollbacks.Count-ne2)){Fail 'Two-payload rollback journal shape differs.'}
...
$tempFiles=@(Get-ChildItem -LiteralPath $mirror -Recurse -Force -File|Where-Object{$_.Name-like'*.ascend950-formal-transaction-*' -and $_.Name-cne'.ascend950-formal-transaction-mirror'});if($tempFiles.Count-ne0){Fail 'Transaction temp files remain in mirror.'}
$lockFiles=@(Get-ChildItem -LiteralPath (Join-Path $PackageRoot 'scripts') -Force -File|Where-Object{$_.Name-like'ASCEND950-FORMAL-TRANSACTION-LOCK-*.lock'});if($lockFiles.Count-ne0){Fail 'Transaction lock files remain.'}
$summary=[ordered]@{scenario=$Scenario;status='passed';executor_exit_code=$code;expected_outcome=$expectedStatus;rollback_succeeded=$rollbackSucceeded;mirror_aggregate_after=$mirrorAggregate;formal_aggregate_before=$beforeFormal;formal_aggregate_after=$afterFormal;journal_rows=$journal.Count;csv_commit_complete=@($journal|Where-Object{$_.phase-ceq'commit_complete'-and$_.kind-ceq'csv_replace'}).Count;csv_rollback_complete=@($journal|Where-Object{$_.phase-ceq'rollback_complete'-and$_.kind-ceq'csv_restore'}).Count;payload_commit_complete=@($journal|Where-Object{$_.phase-ceq'commit_complete'-and$_.kind-ceq'payload_copy'}).Count;payload_rollback_complete=@($journal|Where-Object{$_.phase-ceq'rollback_complete'-and$_.kind-ceq'payload_delete'}).Count;endpoint_files_seeded=$endpointCopies;reverse_csv_order_verified=$reverseCsvVerified;reverse_payload_order_verified=$reversePayloadVerified;temp_files_remaining=$tempFiles.Count;lock_files_remaining=$lockFiles.Count;formal_writes_performed=0}
...
Write-Output "PASS: Ascend 950 rehearsal scenario $Scenario; formal root unchanged."
