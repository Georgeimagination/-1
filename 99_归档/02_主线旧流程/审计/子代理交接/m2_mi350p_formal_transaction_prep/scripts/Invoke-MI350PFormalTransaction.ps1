...
$ExpectedCandidatePosthashSha256 = '49e57afa591d265717ca1bc08bb59d4efd86c81f6a571b4715f22226907b0074'
$ExpectedToken = 'MI350P-FORMAL-MERGE-20260813-74AC69C2-F1520907'
$IsolatedMarkerText = 'M2-W3-AMD-MI350P-ISOLATED-TRANSACTION'
...
$Preflight = Join-Path $WorkRoot 'Test-MI350PFormalTransactionPreflight.ps1'
$ReplayEngine = Join-Path $WorkRoot 'Replay-MI350PAuthorizedRows.ps1'
...
$ExpectedCandidatePosthashPath = Join-Path $PackageRoot 'expected-candidate-19-table-posthashes.csv'
...
Assert-Check ($backupList.Count -eq 19) 'Backup-candidate list is not 19 rows.'
Assert-Check ($guardList.Count -eq 3) 'Guard list is not three rows.'
Assert-Check ($newFileList.Count -eq 4) 'New-file target list is not four rows.'
...
if ($isFormalRoot) {
    ...
    Assert-Check ($transactionReviewRow.transaction_review_id -ceq 'MI350P-FORMAL-TRANSACTION-REVIEW-20260813') 'Transaction-review ID differs.'
    Assert-Check ($transactionReviewRow.package_id -ceq 'M2-W3-AMD-MI350P-FORMAL-TRANSACTION-V1') 'Transaction-review package ID differs.'
    Assert-Check ($transactionReviewRow.reviewer -ceq 'm2_mi350p_formal_transaction_independent_review') 'Transaction-review reviewer identity differs.'
    Assert-Check ($transactionReviewRow.reviewer -cne 'm2_mi350p_transaction_prep') 'Transaction-prep author cannot independently authorize formal execution.'
    Assert-Check ($transactionReviewRow.review_date -ceq '2026-08-13') 'Transaction-review date differs.'
    Assert-Check ($transactionReviewRow.authorization_scope -ceq 'formal_execution_of_exact_274_row_signoff_via_transaction_v1') 'Transaction-review authorization scope differs.'
    ...
    $rehearsalScript = Join-Path $WorkRoot 'Test-MI350PFormalTransactionRehearsal.ps1'
    ...
    Assert-Check ($transactionReviewRow.final_review_report_sha256 -ceq $ExpectedFinalReportSha256) 'Transaction-review final-report binding differs.'
    Assert-Check ($transactionReviewRow.mi350p_signoff_sha256 -ceq $ExpectedSignoffSha256) 'Transaction-review MI350P signoff binding differs.'
    Assert-Check ($transactionReviewRow.freeze_manifest_sha256 -ceq $ExpectedFreezeSha256) 'Transaction-review MI350P content-review binding differs.'
    ...
    Assert-Check ($transactionReviewRow.candidate_posthash_manifest_sha256 -ceq $ExpectedCandidatePosthashSha256) 'Transaction-review candidate-posthash binding differs.'
    Assert-Check ($transactionReviewRow.candidate_aggregate_sha256 -ceq $ExpectedCandidateAggregate) 'Transaction-review candidate aggregate binding differs.'
    Assert-Check ([string]$transactionReviewRow.candidate_validator_checks -ceq [string]$ExpectedValidatorChecks) 'Transaction-review candidate validator count differs.'
    ...
    $backupManifestPath = Join-Path $BackupRoot 'MI350P-FORMAL-BACKUP-MANIFEST.csv'
    ...
    Assert-Check ($backupManifest.Count -eq 23) "Formal backup manifest row count differs: $($backupManifest.Count)."
    Assert-Check (@($backupManifest.relative_path | Sort-Object -Unique).Count -eq 23) 'Formal backup manifest paths are not unique.'
    $expectedBackupPaths = @($backupList.target_path) + @($guardList.target_path | Sort-Object -Unique) + @('MI350P-NEW-FILE-TARGETS-ABSENT.csv','MI350P-CANDIDATE-19-TABLE-POSTHASHES.csv')
    ...
    $absenceBackup = Get-ChildPath -Root $BackupRoot -RelativePath 'MI350P-NEW-FILE-TARGETS-ABSENT.csv' -MustExist $true
    Assert-Check ((Get-Sha256 $absenceBackup) -ceq (Get-Sha256 $NewFileListPath)) 'Formal backup absent-target list differs.'
    $posthashBackup = Get-ChildPath -Root $BackupRoot -RelativePath 'MI350P-CANDIDATE-19-TABLE-POSTHASHES.csv' -MustExist $true
    ...
    $authorizationMarker = Join-Path $BackupRoot 'MI350P-FORMAL-TRANSACTION-AUTHORIZATION.txt'
    Assert-Check (Test-Path -LiteralPath $authorizationMarker -PathType Leaf) 'Formal BackupRoot lacks MI350P-FORMAL-TRANSACTION-AUTHORIZATION.txt.'
...
if (-not $isFormalRoot) {
    $markerPath = Join-Path $TargetRoot '.mi350p-formal-transaction-mirror'
    Assert-Check (Test-Path -LiteralPath $markerPath -PathType Leaf) 'Non-formal TargetRoot lacks .mi350p-formal-transaction-mirror.'
    $markerText = [IO.File]::ReadAllText($markerPath, [Text.Encoding]::UTF8).Trim()
    Assert-Check ($markerText -ceq 'MI350P-FORMAL-TRANSACTION-MIRROR-20260813') 'Non-formal TargetRoot marker content differs.'
}
...
$transactionContainer = if ($isFormalRoot) { $BackupRoot } else { $PackageRoot }
$transactionRoot = Join-Path $transactionContainer ('.tmp-mi350p-formal-' + [guid]::NewGuid().ToString('N'))
...
if (-not [string]::IsNullOrWhiteSpace($env:MI350P_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS)) { $FaultAfterReplacements = [int]$env:MI350P_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS }
if (-not [string]::IsNullOrWhiteSpace($env:MI350P_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS)) { $FaultAfterPayloads = [int]$env:MI350P_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS }
...
$lockPath = Join-Path $lockRoot ('MI350P-FORMAL-TRANSACTION-LOCK-' + $lockKey + '.lock')
$lockStream = $null
$lockOwned = $false
try {
...
try {
    ...
    [IO.File]::WriteAllText((Join-Path $candidateRoot '.mi350p-merge-isolated-target'), $IsolatedMarkerText, (New-Object Text.UTF8Encoding($false)))
    ...
    Assert-Check ([int]$replayResult.authorized_writes -eq 270) 'Accepted replay engine did not execute 270 writes.'
    Assert-Check ([int]$replayResult.files_created -eq 4) 'Accepted replay engine did not copy four transaction files.'
    ...
    Assert-Check ($candidateAggregate -ceq $ExpectedCandidateAggregate) 'Candidate 32-table aggregate differs from the frozen expected aggregate.'
    ...
    foreach ($item in $backupList) {
        ...
        $tempReplacement = $target + '.mi350p-formal-transaction-new'
    ...
    foreach ($item in $backupList) {
        $target = Get-ChildPath -Root $TargetRoot -RelativePath $item.target_path -MustExist $true
        Assert-Check (-not (Test-Path -LiteralPath ($target + '.mi350p-formal-transaction-new'))) "Replacement temp remains after success: $($item.target_path)"
        Assert-Check (-not (Test-Path -LiteralPath ($target + '.mi350p-formal-transaction-restore'))) "Restore temp remains after success: $($item.target_path)"
    }
    ...
    Add-JournalEvent -Phase 'commit_complete' -Kind 'transaction' -RelativePath '' -ObservedSha256 $candidateHashManifestSha256 -Details '270_writes_3_guards_111390_checks'
    ...
    try {
        ...
        for ($index = $replacedPaths.Count - 1; $index -ge 0; $index--) {
            ...
            $restoreTemp = $target + '.mi350p-formal-transaction-restore'
        ...
        foreach ($item in $backupList) {
            $target = Get-ChildPath -Root $TargetRoot -RelativePath $item.target_path -MustExist $true
            $replacementTemp = $target + '.mi350p-formal-transaction-new'
            $restoreTemp = $target + '.mi350p-formal-transaction-restore'
