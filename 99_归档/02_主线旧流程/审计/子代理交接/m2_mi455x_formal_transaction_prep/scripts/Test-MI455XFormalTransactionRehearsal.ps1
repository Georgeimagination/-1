...
param(
    ...
    [string]$PackageRoot = 'D:\Workspace\2026projects\调研-训练推理芯片调研-资料汇总\审计\子代理交接\m2_mi455x_formal_transaction_prep'
)
Set-StrictMode -Version 2.0
$ErrorActionPreference='Stop'
$Utf8NoBom=New-Object Text.UTF8Encoding($false)
$ExpectedBaseline='f1520907d54e406e7d82f591e3799f07f569218aff0c321ac3b836b6c649da10'
$ExpectedPostmerge='0d94c60e136fab58941ad8d1f6b5b55a4d2013a370c9a1dc938bd8005fa57bca'
...
[IO.File]::WriteAllText((Join-Path $mirror '.mi455x-formal-transaction-mirror'),'MI455X-FORMAL-TRANSACTION-MIRROR-20260813',$Utf8NoBom)
...
$beforeFormal=Get-Aggregate $SourceRoot $manifest;$executor=Join-Path $PackageRoot 'scripts\Invoke-MI455XFormalTransaction.ps1'
$env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS='';$env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS=''
if($Scenario-ceq'rollback_after_3_tables'){$env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS='3'}
if($Scenario-ceq'rollback_after_2_payloads'){$env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS='2'}
...
$tempFiles=@(Get-ChildItem -LiteralPath $mirror -Recurse -Force -File|Where-Object{$_.Name-like'*.mi455x-formal-transaction-*' -and $_.Name-cne'.mi455x-formal-transaction-mirror'});if($tempFiles.Count-ne0){Fail 'Transaction temp files remain in mirror.'}
...
Write-Output "PASS: MI455X rehearsal scenario $Scenario; formal root unchanged."
