...
$ExpectedCandidatePosthashSha256 = '7f9425cae6b74fc5af5e6c6fbb86815f339ff8ef3d112f0ad87bc0aab8f6d85f'
$ExpectedToken = 'MI455X-FORMAL-MERGE-20260813-19686303-F1520907'
$IsolatedMarkerText = 'M2-W3-AMD-MI455X-ISOLATED-TRANSACTION'
...
$Preflight = Join-Path $WorkRoot 'Test-MI455XFormalTransactionPreflight.ps1'
$ReplayEngine = Join-Path $WorkRoot 'Replay-MI455XAuthorizedRows.ps1'
...
Assert-Check ($newFileList.Count -eq 3) 'New-file target list is not three rows.'
...
if ($isFormalRoot) {
    ...
    Assert-Check ($transactionReviewRow.transaction_review_id -ceq 'MI455X-FORMAL-TRANSACTION-REVIEW-20260813') 'Transaction-review ID differs.'
    Assert-Check ($transactionReviewRow.package_id -ceq 'M2-W3-AMD-MI455X-FORMAL-TRANSACTION-V1') 'Transaction-review package ID differs.'
    Assert-Check ($transactionReviewRow.reviewer -ceq 'm2_mi455x_formal_transaction_independent_review') 'Transaction-review reviewer identity differs.'
    Assert-Check ($transactionReviewRow.reviewer -cne 'm2_mi455x_transaction_prep') 'Transaction-prep author cannot independently authorize formal execution.'
    Assert-Check ($transactionReviewRow.review_date -ceq '2026-08-13') 'Transaction-review date differs.'
    Assert-Check ($transactionReviewRow.authorization_scope -ceq 'formal_execution_of_exact_245_row_signoff_via_transaction_v1') 'Transaction-review authorization scope differs.'
    ...
    $rehearsalScript = Join-Path $WorkRoot 'Test-MI455XFormalTransactionRehearsal.ps1'
    ...
    Assert-Check ($transactionReviewRow.mi455x_signoff_sha256 -ceq $ExpectedSignoffSha256) 'Transaction-review MI455X signoff binding differs.'
    Assert-Check ($transactionReviewRow.rebase_freeze_manifest_sha256 -ceq $ExpectedRebaseFreezeSha256) 'Transaction-review MI455X content-review binding differs.'
    ...
    $backupManifestPath = Join-Path $BackupRoot 'MI455X-FORMAL-BACKUP-MANIFEST.csv'
    ...
    $expectedBackupPaths = @($backupList.target_path) + @($guardList.target_path | Sort-Object -Unique) + @('MI455X-NEW-FILE-TARGETS-ABSENT.csv','MI455X-CANDIDATE-19-TABLE-POSTHASHES.csv')
    ...
    $absenceBackup = Get-ChildPath -Root $BackupRoot -RelativePath 'MI455X-NEW-FILE-TARGETS-ABSENT.csv' -MustExist $true
    Assert-Check ((Get-Sha256 $absenceBackup) -ceq (Get-Sha256 $NewFileListPath)) 'Formal backup absent-target list differs.'
    $posthashBackup = Get-ChildPath -Root $BackupRoot -RelativePath 'MI455X-CANDIDATE-19-TABLE-POSTHASHES.csv' -MustExist $true
    ...
    $authorizationMarker = Join-Path $BackupRoot 'MI455X-FORMAL-TRANSACTION-AUTHORIZATION.txt'
    Assert-Check (Test-Path -LiteralPath $authorizationMarker -PathType Leaf) 'Formal BackupRoot lacks MI455X-FORMAL-TRANSACTION-AUTHORIZATION.txt.'
...
if (-not $isFormalRoot) {
    $markerPath = Join-Path $TargetRoot '.mi455x-formal-transaction-mirror'
    Assert-Check (Test-Path -LiteralPath $markerPath -PathType Leaf) 'Non-formal TargetRoot lacks .mi455x-formal-transaction-mirror.'
    $markerText = [IO.File]::ReadAllText($markerPath, [Text.Encoding]::UTF8).Trim()
    Assert-Check ($markerText -ceq 'MI455X-FORMAL-TRANSACTION-MIRROR-20260813') 'Non-formal TargetRoot marker content differs.'
}
...
$transactionRoot = Join-Path $transactionContainer ('.tmp-mi455x-formal-' + [guid]::NewGuid().ToString('N'))
...
if (-not [string]::IsNullOrWhiteSpace($env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS)) { $FaultAfterReplacements = [int]$env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_REPLACEMENTS }
if (-not [string]::IsNullOrWhiteSpace($env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS)) { $FaultAfterPayloads = [int]$env:MI455X_FORMAL_TRANSACTION_TEST_FAIL_AFTER_PAYLOADS }
...
$lockPath = Join-Path $lockRoot ('MI455X-FORMAL-TRANSACTION-LOCK-' + $lockKey + '.lock')
$lockStream = $null
$lockOwned = $false
try {
...
try {
    ...
    [IO.File]::WriteAllText((Join-Path $candidateRoot '.mi455x-merge-isolated-target'), $IsolatedMarkerText, (New-Object Text.UTF8Encoding($false)))
    ...
    Assert-Check ([int]$replayResult.authorized_writes -eq 242) 'Accepted replay engine did not execute 242 writes.'
    Assert-Check ([int]$replayResult.files_created -eq 3) 'Accepted replay engine did not copy three transaction files.'
    ...
    foreach ($item in $backupList) {
        ...
        $tempReplacement = $target + '.mi455x-formal-transaction-new'
    ...
    foreach ($item in $backupList) {
        $target = Get-ChildPath -Root $TargetRoot -RelativePath $item.target_path -MustExist $true
        Assert-Check (-not (Test-Path -LiteralPath ($target + '.mi455x-formal-transaction-new'))) "Replacement temp remains after success: $($item.target_path)"
        Assert-Check (-not (Test-Path -LiteralPath ($target + '.mi455x-formal-transaction-restore'))) "Restore temp remains after success: $($item.target_path)"
    }
    ...
    Add-JournalEvent -Phase 'commit_complete' -Kind 'transaction' -RelativePath '' -ObservedSha256 $candidateHashManifestSha256 -Details '242_writes_3_guards_111040_checks'
    ...
    try {
        ...
        for ($index = $replacedPaths.Count - 1; $index -ge 0; $index--) {
            ...
            $restoreTemp = $target + '.mi455x-formal-transaction-restore'
        ...
        foreach ($item in $backupList) {
            $target = Get-ChildPath -Root $TargetRoot -RelativePath $item.target_path -MustExist $true
            $replacementTemp = $target + '.mi455x-formal-transaction-new'
            $restoreTemp = $target + '.mi455x-formal-transaction-restore'
