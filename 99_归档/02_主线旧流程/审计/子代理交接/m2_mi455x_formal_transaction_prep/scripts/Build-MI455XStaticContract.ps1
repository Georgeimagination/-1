param(
    ...
    [Parameter(Mandatory=$false)][string]$OutputRoot = 'D:\Workspace\2026projects\调研-训练推理芯片调研-资料汇总\审计\子代理交接\m2_mi455x_formal_transaction_prep'
)
...
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $rootFull '审计\子代理交接\m2_mi455x_formal_transaction_prep')).TrimEnd('\')
...
$rebaseDir = '审计/子代理交接/m2_mi455x_rebase'
$signoffPath = Get-ChildPath $rootFull ($rebaseDir + '/m2_mi455x_rebase_signoff.csv')
$rebaseFreezePath = Get-ChildPath $rootFull ($rebaseDir + '/rebase-freeze-manifest.csv')
$formalManifestJsonPath = Get-ChildPath $rootFull ($rebaseDir + '/formal-32-current-baseline.json')
$originalFreezePath = Get-ChildPath $rootFull '审计/子代理交接/m2_staging/M2-W3-AMD-MI455X-MODULE/validation/freeze-manifest.csv'
Assert-Check ((Get-Sha256 $signoffPath) -ceq $ExpectedSignoffSha256) 'Rebase signoff SHA-256 differs.'
Assert-Check ((Get-Sha256 $rebaseFreezePath) -ceq $ExpectedRebaseFreezeSha256) 'Rebase freeze-manifest SHA-256 differs.'
Assert-Check ((Get-Sha256 $formalManifestJsonPath) -ceq $ExpectedFormalManifestSha256) 'Formal baseline manifest SHA-256 differs.'
Assert-Check ((Get-Sha256 $originalFreezePath) -ceq $ExpectedOriginalFreezeSha256) 'Original staging freeze-manifest SHA-256 differs.'

$rebaseFreeze = @(Import-Csv -LiteralPath $rebaseFreezePath -Encoding UTF8)
Assert-Check ($rebaseFreeze.Count -eq 61) "Rebase freeze row count differs: $($rebaseFreeze.Count)."
Assert-Check (@($rebaseFreeze.relative_path | Group-Object | Where-Object Count -gt 1).Count -eq 0) 'Rebase freeze contains duplicate paths.'
foreach ($entry in $rebaseFreeze) {
    $member = Get-ChildPath $rootFull ([string]$entry.relative_path)
    Assert-Check ((Get-Sha256 $member) -ceq [string]$entry.sha256) "Rebase freeze member hash differs: $($entry.relative_path)"
    Assert-Check ([string](Get-Item -LiteralPath $member).Length -ceq [string]$entry.bytes) "Rebase freeze member size differs: $($entry.relative_path)"
}
$rebaseFreezeAggregate = Get-ManifestAggregate -Rows $rebaseFreeze -IncludeBytes $true
Assert-Check ($rebaseFreezeAggregate -ceq $ExpectedRebaseFreezeAggregate) "Rebase freeze aggregate differs: $rebaseFreezeAggregate"

$originalFreeze = @(Import-Csv -LiteralPath $originalFreezePath -Encoding UTF8)
$stagingBase = Join-Path $rootFull '审计\子代理交接\m2_staging\M2-W3-AMD-MI455X-MODULE'
Assert-Check (@($originalFreeze.relative_path | Group-Object | Where-Object Count -gt 1).Count -eq 0) 'Original freeze contains duplicate paths.'
foreach ($entry in $originalFreeze) {
    $member = Get-ChildPath $stagingBase ([string]$entry.relative_path)
    Assert-Check ((Get-Sha256 $member) -ceq [string]$entry.sha256) "Original freeze member hash differs: $($entry.relative_path)"
    Assert-Check ([string](Get-Item -LiteralPath $member).Length -ceq [string]$entry.bytes) "Original freeze member size differs: $($entry.relative_path)"
}
$originalFreezeAggregate = Get-ManifestAggregate -Rows $originalFreeze -IncludeBytes $true
Assert-Check ($originalFreezeAggregate -ceq $ExpectedOriginalFreezeAggregate) "Original freeze aggregate differs: $originalFreezeAggregate"

$signoff = @(Import-Csv -LiteralPath $signoffPath -Encoding UTF8)
Assert-Check ($signoff.Count -eq 245) "Signoff row count differs: $($signoff.Count)."
...
Assert-Check ($writes.Count -eq 242) "Structured signoff count differs: $($writes.Count)."
Assert-Check ($artifacts.Count -eq 3) "Artifact signoff count differs: $($artifacts.Count)."
foreach ($binding in $signoff) {
    Assert-Check ([string]$binding.freeze_manifest_sha256 -ceq $ExpectedOriginalFreezeSha256) "Original freeze binding differs: $($binding.signoff_row_id)"
    Assert-Check ([string]$binding.freeze_aggregate_sha256 -ceq $ExpectedOriginalFreezeAggregate) "Original freeze aggregate binding differs: $($binding.signoff_row_id)"
    ...
    Assert-Check ([string]$binding.reviewer -ceq 'm2_mi455x_rebase') "Signoff reviewer differs: $($binding.signoff_row_id)"
}

$pkAuditPath = Get-ChildPath $rootFull ($rebaseDir + '/pk-conflict-audit.csv')
$pkAudit = @(Import-Csv -LiteralPath $pkAuditPath -Encoding UTF8)
Assert-Check ($pkAudit.Count -eq 242) "PK audit row count differs: $($pkAudit.Count)."
Assert-Check (@($pkAudit | Where-Object { $_.result -cne 'no_conflict' }).Count -eq 0) 'PK audit contains a conflict.'
Assert-Check (@($pkAudit | Where-Object { $_.source_line_hash_match -cne 'yes' }).Count -eq 0) 'Legacy source-line audit contains a mismatch.'
...
foreach ($binding in ($writes | Sort-Object signoff_row_id)) {
    ...
    $writeSet.Add([pscustomobject][ordered]@{
        ...
        canonical_source_row_sha256 = Get-CanonicalRowSha256 $sourceRow
...
$guardSpecs = @(
    [pscustomobject]@{ guard_id='GUARD-MI455X-OBJECT'; target_path='数据/objects.csv'; pk_column='object_id'; pk_value='OBJ-AMD-MI455X'; reason='MI455X object already exists and is outside the 242-row append write-set.' },
    [pscustomobject]@{ guard_id='GUARD-CDNA5-OBJECT'; target_path='数据/objects.csv'; pk_column='object_id'; pk_value='OBJ-AMD-CDNA5-ARCH'; reason='CDNA 5 architecture object must remain unchanged while implementation facts are appended.' },
    [pscustomobject]@{ guard_id='GUARD-MI455X-CDNA5-RELATION'; target_path='数据/object-relations.csv'; pk_column='object_relation_id'; pk_value='OREL-AMD-MI455X-IMPLEMENTS-CDNA5'; reason='The pre-existing implements_architecture relation is not part of the write-set.' }
)
...
Assert-Check ($artifactRows.Count -eq 3) 'New-file target count is not three.'
...
foreach ($row in $artifactRows) {
    $rollbackRows.Add([pscustomobject][ordered]@{
        inventory_role='new_file'; commit_order=[string](19+[int]$row.commit_order); rollback_order=[string](4-[int]$row.commit_order); target_path=[string]$row.target_path; precondition='target_absent'; backup_content='absence_sentinel'; rollback_action='delete_only_if_exact_committed_hash'; irreversible='false'
    })
}
...
$inputAudit = [ordered]@{
    ...
    rebase_freeze_manifest_sha256 = Get-Sha256 $rebaseFreezePath
    rebase_freeze_rows = $rebaseFreeze.Count
    rebase_freeze_aggregate = $rebaseFreezeAggregate
    original_freeze_manifest_sha256 = Get-Sha256 $originalFreezePath
    original_freeze_rows = $originalFreeze.Count
    original_freeze_aggregate = $originalFreezeAggregate
    legacy_source_line_audit_rows = $pkAudit.Count
    legacy_source_line_audit_failures = @($pkAudit | Where-Object { $_.source_line_hash_match -cne 'yes' }).Count
...
Write-Output "PASS: MI455X static transaction contract; $Checks checks; 242 rows across 19 tables; 3 absent file targets; formal aggregate unchanged."
