# NVIDIA GA100 die 资料卡候选

> 交付状态：`content candidate / accept_with_fixes`，未正式发布，未写入正式 CSV，也未形成正式 selection run。  
> 资料截止日：2026-08-21。该日期是本候选卡的核查元数据，不是 GA100 芯片事实。  
> 对象标识：候选 `OBJ-NVIDIA-GA100-DIE`。  
> 对象层级：`silicon_package / die` 阅读层，实际研究对象是完整 128-SM GA100 裸片设计。  
> 架构关系：候选 `OREL-NVIDIA-GA100-IMPLEMENTS-AMPERE`，指向 `OBJ-NVIDIA-AMPERE-ARCH`。  
> 来源规模：正文索引 23 个来源家族，其中 19 个进入内容层最小集，S08、S11、S12、S13 四个只作 lead-only 展示或审计线索。不同修订、镜像和同一提交下的多个 endpoint 不增加独立来源数。  
> 引用方法：正文使用 `[Sxx, 定位]`。第 14 节给出每个短标签对应的来源 ID、endpoint 和版本边界。
...
| 来源证据 | `partial` | 内容已覆盖 23 个 family，并保留 endpoint/locator；多项 ID 与 payload 仍在 staging，未正式注册、筛选和反向移除 |
...
复核结论：内容可以作为 GA100 full-die 资料卡的人工审阅候选。正式发布仍需完成 source/version/endpoint 入库、原子 assertion 与 missing-evidence chain、对象级 reverse removal、正式 card completeness、独立复核和 Windows 三道数据门。本文件本身不宣称这些发布动作已经完成。
