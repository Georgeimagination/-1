...
param(
    ...
    [string]$SubjectContractMode = 'gate'
)

$ErrorActionPreference = 'Stop'
$script:Errors = [System.Collections.Generic.List[string]]::new()
$script:Checks = 0
$script:SubjectContractErrors = [System.Collections.Generic.List[string]]::new()
...
function Add-SubjectContractError {
    param([string]$Message)
    $script:SubjectContractErrors.Add($Message)
    if ($SubjectContractMode -eq 'gate') { $script:Errors.Add($Message) }
}
function Add-SubjectContractStructureError {
    param([string]$Message)
    $script:SubjectContractErrors.Add($Message)
    $script:Errors.Add($Message)
}
...
# Subject contracts separate fact subjects from requirement coverage targets.
$contractKinds = @('object','component','link','object_relation','precision_path','capability','topology')
$schemaHasRequirementContract = @($schema | Where-Object { $_.table_path -eq '数据/fields.csv' -and $_.column_name -eq 'allowed_requirement_target_kinds' }).Count -eq 1
$script:Checks++
if (-not $schemaHasRequirementContract) { Add-SubjectContractStructureError 'Schema is missing 数据/fields.csv.allowed_requirement_target_kinds.' }
$targetColumnByKind = [ordered]@{
    object = 'object_id'
    component = 'component_id'
    link = 'link_id'
    object_relation = 'object_relation_id'
    precision_path = 'precision_path_id'
    capability = 'capability_id'
    topology = 'topology_id'
}
function Get-ContractParts {
    param([string]$Value, [string]$Context)
    $script:Checks++
    if (Test-Empty $Value) {
        Add-SubjectContractStructureError "Subject contract empty: $Context"
        return @()
    }
    $parts = @($Value.Split(';') | ForEach-Object { $_.Trim() })
    $script:Checks++
    if (@($parts | Where-Object { Test-Empty $_ }).Count -gt 0) { Add-SubjectContractStructureError "Subject contract has empty token: $Context='$Value'" }
    $script:Checks++
    if (@($parts | Sort-Object -Unique).Count -ne $parts.Count) { Add-SubjectContractStructureError "Subject contract has duplicate kind: $Context='$Value'" }
    $script:Checks++
    if (@($parts | Where-Object { $_ -notin $contractKinds }).Count -gt 0) { Add-SubjectContractStructureError "Subject contract has unknown kind: $Context='$Value'" }
    $expected = @($contractKinds | Where-Object { $_ -in $parts })
    $script:Checks++
    if (($parts -join ';') -cne ($expected -join ';')) { Add-SubjectContractStructureError "Subject contract order is not canonical: $Context='$Value'; expected='$($expected -join ';')'" }
    return $parts
}
function Get-TargetKind {
    param($Row, [string]$Context)
    $nonEmpty = @($targetColumnByKind.GetEnumerator() | Where-Object { -not (Test-Empty $Row.($_.Value)) })
    $script:Checks++
    if ($nonEmpty.Count -ne 1) {
        Add-SubjectContractError "Subject contract target cardinality is not one: $Context"
        return ''
    }
    return [string]$nonEmpty[0].Key
}
function Get-TargetFingerprint {
    param($Row)
    return @($targetColumnByKind.Values | ForEach-Object { [string]$Row.$_ }) -join [char]31
}

if ($tableData.ContainsKey('数据/fields.csv')) {
    $fieldRows = @($tableData['数据/fields.csv'])
    $contractByField = @{}
    foreach ($field in $fieldRows) {
        $factContract = @(Get-ContractParts -Value ([string]$field.allowed_subject_kinds) -Context "数据/fields.csv:$($field.field_id).allowed_subject_kinds")
        $requirementValue = [string]$field.allowed_requirement_target_kinds
        if (Test-Empty $requirementValue) {
            Add-SubjectContractStructureError "Subject contract column missing or empty: 数据/fields.csv:$($field.field_id).allowed_requirement_target_kinds"
            $requirementValue = [string]$field.allowed_subject_kinds
        }
        $requirementContract = @(Get-ContractParts -Value $requirementValue -Context "数据/fields.csv:$($field.field_id).allowed_requirement_target_kinds")
        $contractByField[$field.field_id] = [pscustomobject]@{ fact=$factContract; requirement=$requirementContract }
    }
    if ($tableData.ContainsKey('数据/facts.csv')) {
        foreach ($fact in $tableData['数据/facts.csv']) {
            $kind = Get-TargetKind -Row $fact -Context "数据/facts.csv:$($fact.fact_id)"
            $script:Checks++
            if (-not $contractByField.ContainsKey($fact.field_id)) {
                Add-SubjectContractError "Fact field contract missing: 数据/facts.csv:$($fact.fact_id) field=$($fact.field_id)"
            } elseif (-not (Test-Empty $kind) -and $kind -notin $contractByField[$fact.field_id].fact) {
                Add-SubjectContractError "Fact subject contract mismatch: 数据/facts.csv:$($fact.fact_id) field=$($fact.field_id) actual=$kind allowed=$($contractByField[$fact.field_id].fact -join ';')"
            }
        }
    }
    if ($tableData.ContainsKey('数据/field-requirements.csv')) {
        $requirementsForContract = @($tableData['数据/field-requirements.csv'])
        $factsForContract = if ($tableData.ContainsKey('数据/facts.csv')) { @($tableData['数据/facts.csv']) } else { @() }
        $relationsForContract = if ($tableData.ContainsKey('数据/object-relations.csv')) { @($tableData['数据/object-relations.csv']) } else { @() }
        foreach ($req in $requirementsForContract) {
            $kind = Get-TargetKind -Row $req -Context "数据/field-requirements.csv:$($req.requirement_id)"
            $script:Checks++
            if (-not $contractByField.ContainsKey($req.field_id)) {
                Add-SubjectContractError "Requirement field contract missing: 数据/field-requirements.csv:$($req.requirement_id) field=$($req.field_id)"
            } elseif (-not (Test-Empty $kind) -and $kind -notin $contractByField[$req.field_id].requirement) {
                Add-SubjectContractError "Requirement target contract mismatch: 数据/field-requirements.csv:$($req.requirement_id) field=$($req.field_id) actual=$kind allowed=$($contractByField[$req.field_id].requirement -join ';')"
            }
            if ($req.requirement_status -in @('value_available','conflicting_unresolved')) {
                $isArchitectureProjection = $req.field_id -eq 'FIELD-ID-ARCH'
                if ($isArchitectureProjection) {
                    $script:Checks++
                    if ($kind -ne 'object_relation') {
                        Add-SubjectContractError "FIELD-ID-ARCH projection must target object_relation: 数据/field-requirements.csv:$($req.requirement_id) actual=$kind"
                    } else {
                        $relation = @($relationsForContract | Where-Object object_relation_id -eq $req.object_relation_id)
                        $script:Checks++
                        if ($relation.Count -ne 1 -or $relation[0].relation_type -ne 'implements_architecture') {
                            Add-SubjectContractError "FIELD-ID-ARCH projection requires a unique implements_architecture relation: 数据/field-requirements.csv:$($req.requirement_id) target=$($req.object_relation_id)"
                        }
                    }
                } else {
                    $reqTarget = Get-TargetFingerprint -Row $req
                    $matches = @($factsForContract | Where-Object { $_.field_id -eq $req.field_id -and (Get-TargetFingerprint -Row $_) -ceq $reqTarget })
                    $script:Checks++
                    if ($matches.Count -eq 0) {
                        Add-SubjectContractError "Requirement lacks same-target same-field fact: 数据/field-requirements.csv:$($req.requirement_id) status=$($req.requirement_status) field=$($req.field_id) target_kind=$kind"
                    }
                }
            }
        }
...
if ($SubjectContractMode -eq 'audit') {
    Write-Output "SUBJECT-CONTRACT AUDIT: $($script:SubjectContractErrors.Count) issue(s); mode=audit."
    foreach ($message in $script:SubjectContractErrors) { Write-Output " - $message" }
}
if ($script:Errors.Count -gt 0) {
    Write-Output "FAIL: $($script:Errors.Count) validation error(s); $($script:Checks) checks executed; subject_contract_mode=$SubjectContractMode."
    foreach ($message in $script:Errors) { Write-Output " - $message" }
    exit 1
}
Write-Output "PASS: 32-table research data model; $($script:Checks) checks executed; subject_contract_mode=$SubjectContractMode."
