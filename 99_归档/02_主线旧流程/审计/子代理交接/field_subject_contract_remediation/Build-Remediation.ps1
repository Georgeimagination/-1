...
$retargets = @{
    ...
    'REQ-AWS-TRN2-0175' = 'PP-AWS-TRN2-CHIP-TENSOR-FP8'
    'REQ-AWS-TRN2-0176' = 'PP-AWS-TRN2-CHIP-TENSOR-BF16'
    'REQ-AWS-TRN2-0177' = 'PP-AWS-TRN2-CHIP-TENSOR-FP32'
    'REQ-AWS-TRN2-0178' = 'PP-AWS-TRN2-CHIP-TENSOR-FP8'
}
...
function Get-SemanticAssessment([object]$preRow) {
    ...
    if ($retargets.ContainsKey($pk)) {
        ...
        if ($pk -like 'REQ-AWS-TRN2-017*') {
            return "该要求原先仍指向 NCv3 单核精度路径，但对应派生事实已经迁到芯片聚合路径 $newTarget。迁移只修正目标，不改变字段、数值状态或生命周期。"
        }
...
function Get-RequirementFingerprint([object]$row, [string]$overrideKind = '', [string]$overrideId = '') {
    $segments = @([string]$row.requirement_id)
    foreach ($kind in $canonicalKindOrder) {
        $column = $targetColumns[$kind]
        $value = [string]$row.$column
        if (-not [string]::IsNullOrWhiteSpace($overrideKind)) {
            if ($kind -eq $overrideKind) { $value = $overrideId } else { $value = '' }
        }
        $segments += $value
    }
    $segments += [string]$row.field_id
...
$fieldProposals = foreach ($fieldId in ($pre.field_id | Sort-Object -Unique)) {
    ...
    elseif ($fieldId -eq 'FIELD-DER-COMPUTE-BW-SPEC') {
        '4 条 precision_path 派生事实各有一条 derived-metrics 记录和两条 derived-inputs；4 条对应要求中 3 条 value_available、1 条 conflicting_unresolved，全部同时迁到对应的 Trainium2 芯片聚合精度路径。'
    }
    else {
...
$overlay = foreach ($pk in ($retargets.Keys | Sort-Object)) {
    ...
    Require ($preRow.Count -eq 1) "Retarget PK is missing or duplicated in preaudit: $pk"
    $preRow = $preRow[0]
    $req = $reqById[$pk]
    $newTarget = $retargets[$pk]
    $newKind = $targetKindById[$newTarget]
    [pscustomobject]@{
        ...
        proposed_target_kind = $newKind
        proposed_target_id = $newTarget
        proposed_field_id = $preRow.field_id
        current_requirement_fingerprint = Get-RequirementFingerprint $req
        proposed_requirement_fingerprint = Get-RequirementFingerprint $req $newKind $newTarget
...
Require ($overlay.Count -eq 12) "Expected 12 overlay rows, got $($overlay.Count)."
Require ((@($adjudication | Where-Object proposed_action -eq 'expand_contract')).Count -eq 12) 'Expected 12 expand_contract rows.'
Require ((@($adjudication | Where-Object proposed_action -eq 'retarget_row')).Count -eq 12) 'Expected 12 retarget_row rows.'
Require ((@($adjudication | Where-Object proposed_action -eq 'no_change_with_rationale')).Count -eq 108) 'Expected 108 no_change_with_rationale rows.'
Require ((@($overlay | Group-Object proposed_requirement_fingerprint | Where-Object Count -ne 1)).Count -eq 0) 'Proposed overlay fingerprints are not unique.'
foreach ($row in $overlay) {
    $req = $reqById[$row.pk]
    Require ($row.current_requirement_fingerprint -eq (Get-RequirementFingerprint $req)) "Current fingerprint mismatch: $($row.pk)"
    Require ($row.proposed_requirement_fingerprint -eq (Get-RequirementFingerprint $req $row.proposed_target_kind $row.proposed_target_id)) "Proposed fingerprint mismatch: $($row.pk)"
}
...
[System.IO.File]::WriteAllText((Join-Path $outDir 'validation-summary.json'), $summaryJson, (New-Object System.Text.UTF8Encoding($true)))
