...
$preflight=Join-Path $WorkRoot 'scripts\Test-Ascend950FormalTransactionPreflight.ps1'
...
try{
    ...
    Write-Output "PASS: Ascend 950 candidate oracle; $ExpectedValidatorChecks checks; aggregate $aggregate; target unchanged."
}
finally{
    if(Test-Path -LiteralPath $oracleFull){Remove-Item -LiteralPath $oracleFull -Recurse -Force}
}
