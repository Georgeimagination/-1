...
Add-EqualityCheck -Name 'PDF manifest row count' -Expected 111 -Actual $pdfRows.Count
...
Add-EqualityCheck -Name 'PDF files on disk' -Expected 111 -Actual $diskPdfFiles.Count
...
Add-EqualityCheck -Name 'PDF total bytes' -Expected ([long]313921821) -Actual $diskTotalBytes
...
if ($null -eq $pdfInfoExe) {
    Add-Check -Name 'PDF page count verification' -Status 'WARN' -Expected 2210 -Actual 'NOT_VERIFIED' -Critical $false -Details ('No explicit pdfinfo.exe was found. ' + $pdfInfoDiscoveryDetail).Trim()
}
else {
    ...
    Add-EqualityCheck -Name 'PDF total pages' -Expected 2210 -Actual $pageTotal
...
Add-EqualityCheck -Name 'Summary unique_pdfs' -Expected 111 -Actual ([int]$summary.unique_pdfs)
